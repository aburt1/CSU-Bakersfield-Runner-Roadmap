...
export default function ApiCheckConfig({ stepId, api }) {
  ...
  const fetchConfig = useCallback(async () => {
    try {
      const data = await api.get(`/steps/${stepId}/api-check`);
      setConfig(data);
      if (data.configured) {
  ...
  const updateHeader = (idx, prop, value) => {
        ...
        <div className="px-4 pb-4 space-y-3">
          ...
          <div className="grid grid-cols-2 gap-3">
            <div>
              ...
              <select value={httpMethod} onChange={(e) => setHttpMethod(e.target.value)} className={field}>
            ...
            <div>
              ...
              <select value={studentParamSource} onChange={(e) => setStudentParamSource(e.target.value)} className={field}>
          ...
          <div>
            ...
            <select value={authType} onChange={(e) => setAuthType(e.target.value)} className={field}>
          ...
          {authType === 'bearer' && (
            <div>
              ...
              <input type="password" value={bearerToken} onChange={(e) => setBearerToken(e.target.value)} className={field} />
            </div>
          )}
