...
export default function TermHeader({ term, canEdit, onSave, onDelete }) {
        ...
        <div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
          ...
          {canEdit && (
            <div className="flex items-center gap-2">
              {!term.is_active && (
                <button
                  onClick={() => onSave(term.id, { is_active: 1 })}
                  className="font-body text-xs font-semibold text-green-700 hover:text-green-900 px-2 py-1 rounded transition-colors"
                >
                  Set Active
                </button>
              )}
              <button
                ...
                className="font-body text-xs text-csub-blue hover:text-csub-blue-dark px-2 py-1 rounded transition-colors"
              >
                Edit Term
              </button>
              <button
                onClick={() => onDelete(term)}
                className="font-body text-xs text-red-600 hover:text-red-800 px-2 py-1 rounded transition-colors"
              >
                Delete Term
              </button>
            </div>
