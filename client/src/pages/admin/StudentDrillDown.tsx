...
export default function StudentDrillDown({ open, onClose, filterType, filterValue, termId, api }: Props) {
        ...
        >
          ...
          >
            ...
            <div className="flex items-start justify-between p-5 border-b border-gray-200">
              <div className="pr-4">
                <h2 className="font-display text-sm font-bold text-csub-blue-dark uppercase tracking-wide">{title}</h2>
                <span className="font-body text-xs text-csub-gray mt-1 block">{total} {total === 1 ? 'student' : 'students'}</span>
              </div>
              <button onClick={onClose} className="p-1 rounded-lg hover:bg-gray-100 text-gray-400 hover:text-gray-600 transition-colors" aria-label="Close">
            ...
            <div className="flex-1 overflow-y-auto p-5">
              {loading ? (
                ...
                <>
                      ...
                      <div key={s.id} className="flex items-center gap-3 p-3 rounded-lg border border-gray-100 hover:bg-gray-50">
                        ...
                        <div className="flex items-center gap-2 flex-shrink-0">
                          <div className="w-16 h-1.5 bg-gray-100 rounded-full overflow-hidden">
                            <div className="h-full bg-csub-blue rounded-full" style={{ width: `${s.completion_pct}%` }} />
                          </div>
                  ...
                  {hasMore && (
                    <button onClick={loadMore} disabled={loadingMore} className="w-full mt-4 py-2.5 font-body text-sm font-semibold text-csub-blue border border-csub-blue/20 rounded-lg hover:bg-csub-blue/5 transition-colors disabled:opacity-50">
