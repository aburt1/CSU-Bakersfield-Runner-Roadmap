...
export default function CloneTermModal({ open, terms, api, defaultSourceTermId, onClose, onCloned }) {
  const [sourceTermId, setSourceTermId] = useState(defaultSourceTermId || terms[0]?.id || null);
  ...
  useEffect(() => {
    ...
    api.get(`/steps?term_id=${sourceTermId}`).then((data) => {
      setSteps(data);
        ...
        <form onSubmit={handleSubmit} className="space-y-5">
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
            <div>
              ...
              <select
                value={sourceTermId || ''}
                onChange={(e) => setSourceTermId(parseInt(e.target.value, 10))}
