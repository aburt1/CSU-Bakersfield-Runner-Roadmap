...
interface Props {
  ...
  terms: TermItem[];
  selectedTermId: number | null;
  onTermsChange: (terms: TermItem[], selectedTermId?: number | null) => void;
        ...
        <div className="flex-1 min-w-0">
          ...
          {step.excluded_tags && (
            <div className="flex flex-wrap gap-1 mt-1">
              ...
              {(typeof step.excluded_tags === 'string' ? JSON.parse(step.excluded_tags) : step.excluded_tags).map((tag: string) => (
...
export default function TermStepsTab({ api, role = 'viewer', terms, selectedTermId, onTermsChange, onSelectTerm }: Props) {
  ...
  const refreshTerms = useCallback(async (nextSelectedTermId: number | null = selectedTermId) => {
  ...
  const handleCreateTerm = async () => {
    ...
    try {
      ...
      const createdTerm = data.find((term: TermItem) => term.id === result.id);
  ...
  const handleSaveTerm = async (termId: number, data: any) => {
    ...
    const term = refreshed.find((item: TermItem) => item.id === termId);
  ...
  const handleDeleteTerm = async (term: TermItem) => {
    ...
    try {
      ...
      const nextTerm = refreshed.find((item: TermItem) => item.is_active) || refreshed[0] || null;
  ...
  const handleCloned = async (result: any) => {
    ...
    const term = refreshed.find((item: TermItem) => item.id === result.term.id);
    onSelectTerm(term?.id || result.term.id);
    setSteps(result.steps || []);
  };

  const stepRowProps = (step: StepItem, index: number) => ({
    step,
    index,
      ...
      <div className="bg-white rounded-xl border border-gray-200 shadow-sm p-5">
        <div className="flex items-center justify-between mb-4">
          ...
          <div className="flex items-center gap-3">
            ...
            {canEdit && (
              <button
                onClick={() => setEditingStep({ term_id: selectedTermId ?? undefined })}
