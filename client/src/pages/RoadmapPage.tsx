...
import ProgressSummary from '../components/roadmap/ProgressSummary.jsx';
import CurrentStepCallout from '../components/roadmap/CurrentStepCallout.jsx';
import RoadmapTimeline from '../components/roadmap/RoadmapTimeline.jsx';
import ListView from '../components/roadmap/ListView.jsx';
import StepDetailPanel from '../components/roadmap/StepDetailPanel.jsx';
import HelpSection from '../components/roadmap/HelpSection.jsx';
import CompletionBanner from '../components/roadmap/CompletionBanner.jsx';
import Celebration from '../components/Celebration.jsx';
import HighContrastToggle from '../components/HighContrastToggle.jsx';
...
export default function RoadmapPage(): React.ReactElement {
  ...
  const [updatingOptionalStepId, setUpdatingOptionalStepId] = useState<number | null>(null);
  ...
  const firstName = (user as { displayName?: string } | null)?.displayName?.split(' ')[0] || 'Student';

  // Filter steps
  const filteredSteps = useMemo(() => {
    if (!showOnlyIncomplete) return steps;
    return steps.filter((s: StepWithStatus) => s.status !== 'completed' && s.status !== 'waived');
  }, [steps, showOnlyIncomplete]);

  const selectedStepList = useMemo(() => {
    if (!selectedStep) return [] as StepWithStatus[];
    return filteredSteps;
  }, [selectedStep, filteredSteps]);

  const handleOptionalStepStatusChange = async (step: StepWithStatus, status: string) => {
    ...
    <div className="min-h-screen bg-gray-50" ref={mainRef}>
      ...
      <main id="main-content" className="max-w-4xl mx-auto px-4 sm:px-6 pb-16">
        ...
        {currentStep && !allComplete && (
          <CurrentStepCallout
            step={currentStep}
            stepNumber={steps.findIndex((s: StepWithStatus) => s.id === currentStep.id) + 1}
          ...
          <div className="flex items-center gap-3">
            ...
            <label className="flex items-center gap-1.5 cursor-pointer">
              <input
                ...
                onChange={(e: React.ChangeEvent<HTMLInputElement>) => setShowOnlyIncomplete(e.target.checked)}
