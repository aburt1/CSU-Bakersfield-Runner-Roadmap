    ...
    <div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
        ...
        <div>
          ...
          <div className="relative mb-3">
            ...
            <div className="absolute inset-y-0 right-0 pr-1.5 flex items-center gap-1">
              ...
              <button
                ...
                disabled={loading}
                className="bg-csub-blue hover:bg-csub-blue-dark text-white font-display font-bold uppercase tracking-wider px-3 py-1.5 rounded-lg shadow transition-colors text-xs disabled:opacity-50"
              >
                {loading ? '...' : 'Search'}
              </button>
            </div>
          ...
          {/* Toolbar: sort + overdue filter */}
          <div className="flex items-center gap-2 mb-3">
            <select
              value={sortBy}
              onChange={(e) => handleSortChange(e.target.value)}
              className="px-3 py-1.5 rounded-lg border border-gray-300 font-body text-xs focus:outline-none focus:ring-2 focus:ring-csub-blue focus:border-transparent bg-white"
            >
            ...
            <button
              ...
              className={`flex items-center gap-1.5 px-3 py-1.5 rounded-lg border font-body text-xs font-medium transition-all ${
                overdueOnly
                  ...
                  : 'border-gray-300 bg-white text-csub-gray hover:border-red-200 hover:text-red-600'
              }`}
            >
          ...
          <div className="space-y-2 max-h-[55vh] overflow-y-auto">
            {!loading && students.length === 0 && (
              <p className="font-body text-sm text-csub-gray text-center py-6">
                {search ? 'No students found.' : 'No students in this term yet.'}
              </p>
            )}
            {loading && students.length === 0 && (
              <p className="font-body text-sm text-csub-gray text-center py-6">Loading students...</p>
            )}
            {students.map((s) => {
          ...
          {totalStudents > 0 && (
            <div className="flex items-center justify-between mt-3 pt-3 border-t border-gray-100">
              <button
                ...
                disabled={page <= 1}
                className="flex items-center gap-1 font-body text-xs font-semibold text-csub-blue hover:text-csub-blue-dark disabled:text-gray-300 disabled:cursor-not-allowed transition-colors"
              >
                <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
              ...
              <span className="font-body text-xs text-csub-gray">
                {totalStudents > 0 ? `${rangeStart}–${rangeEnd} of ${totalStudents}` : '0 results'}
              </span>
              <button
        ...
        <div>
          <StudentDetail
            ...
            api={api}
            role={role}
            onProgressChange={refreshStudents}
          />
        </div>
      </div>
    </div>
