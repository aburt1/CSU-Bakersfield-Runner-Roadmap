...
import type { Student, Step, StudentProgress, AdminUser, Term, AuditLogEntry } from '../types/models.js';
...
router.get('/analytics/cohort-summary', async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    const totalActiveStepsResult = await req.db.queryOne<{ count: string }>(
...
router.get('/analytics/students', async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    switch (filterType) {
      case 'step_completed': {
        const step = await req.db.queryOne<{ title: string }>('SELECT title FROM steps WHERE id = $1', [filterValue]);
