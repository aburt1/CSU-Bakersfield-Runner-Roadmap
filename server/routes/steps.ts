...
function stepAppliesToStudent(step: Step, studentTags: string[]): boolean {
  const requiredTags: string[] = safeJsonParse(step.required_tags, []);
  const excludedTags: string[] = safeJsonParse(step.excluded_tags, []);
...
router.get('/', async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    if (studentId) {
      const student = await req.db.queryOne<{ term_id: number | null }>('SELECT term_id FROM students WHERE id = $1', [studentId]);
      if (student?.term_id) {
        steps = await req.db.queryAll<Step>(
          'SELECT * FROM steps WHERE (is_active = 1 OR is_active IS NULL) AND term_id = $1 ORDER BY sort_order',
          [student.term_id]
        );
      } else {
        const activeTerm = await req.db.queryOne<{ id: number }>('SELECT id FROM terms WHERE is_active = 1 ORDER BY id DESC LIMIT 1');
        steps = activeTerm
          ? await req.db.queryAll<Step>(
              'SELECT * FROM steps WHERE (is_active = 1 OR is_active IS NULL) AND term_id = $1 ORDER BY sort_order',
              [activeTerm.id]
            )
          : await req.db.queryAll<Step>(
...
router.get('/progress', authMiddleware, async (req: Request, res: Response, next: NextFunction) => {
  try {
    const progress = await req.db.queryAll<{ step_id: number; completed_at: string | null; status: string }>(
    ...
    const student = await req.db.queryOne<Pick<Student, 'tags' | 'applicant_type' | 'major' | 'residency' | 'term_id'>>(
      'SELECT tags, applicant_type, major, residency, term_id FROM students WHERE id = $1',
      [req.studentId]
    );

    // Get term info
    let term: { id: number; name: string; start_date: string | null; end_date: string | null } | null = null;
    if (student?.term_id) {
      term = await req.db.queryOne<{ id: number; name: string; start_date: string | null; end_date: string | null }>(
...
router.put('/:stepId/status', authMiddleware, async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    const step = await req.db.queryOne<Step>(
