...
router.get('/steps/:id/api-check', async (req: Request, res: Response) => {
  try {
    const stepId = req.params.id as string;
    const check = await req.db.queryOne<StepApiCheck>(
    ...
    // Mask credentials
    const result: Record<string, unknown> = { ...check, configured: true };
    if (result.auth_credentials) {
      result.auth_credentials = MASKED;
    }
    if (result.headers) {
      try {
        result.headers = JSON.parse(result.headers as string);
      } catch { /* return as-is */ }
    }
...
router.put('/steps/:id/api-check', async (req: Request, res: Response) => {
  try {
    const stepId = req.params.id as string;
    ...
    if (aType !== 'none' && auth_credentials && auth_credentials !== MASKED) {
      ...
      const existing = await req.db.queryOne<{ auth_credentials: string | null }>(
...
router.delete('/steps/:id/api-check', async (req: Request, res: Response) => {
  try {
    const stepId = req.params.id as string;
    await req.db.execute(
...
router.post('/steps/:id/api-check/test', async (req: Request, res: Response) => {
  try {
    const stepId = req.params.id as string;
