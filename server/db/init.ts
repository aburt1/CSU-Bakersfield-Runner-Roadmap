...
export async function initDatabase(): Promise<Db> {
  ...
  const latestTerm = await db.queryOne<{ id: number }>('SELECT id FROM terms ORDER BY id DESC LIMIT 1');
  if (latestTerm?.id) {
  ...
  const termCount = await db.queryOne<{ count: string }>('SELECT COUNT(*) as count FROM terms');
  ...
  const adminCount = await db.queryOne<{ count: string }>('SELECT COUNT(*) as count FROM admin_users');
  ...
  const integrationCount = await db.queryOne<{ count: string }>('SELECT COUNT(*) as count FROM integration_clients');
  ...
  const count = await db.queryOne<{ count: string }>('SELECT COUNT(*) as count FROM steps');
  if (parseInt(count!.count) === 0) {
    const defaultTerm = await db.queryOne<{ id: number }>('SELECT id FROM terms WHERE is_active = 1 ORDER BY id LIMIT 1');
  ...
  try {
    ...
    console.warn('[db-init] Unable to create unique Student ID # index:', message);
  }
  ...
  const studentCount = await db.queryOne<{ count: string }>('SELECT COUNT(*) as count FROM students');
  if (parseInt(studentCount!.count) === 0 && process.env.NODE_ENV !== 'production') {
    const defaultTerm = await db.queryOne<{ id: number }>('SELECT id FROM terms WHERE is_active = 1 ORDER BY id LIMIT 1');
    const termId = defaultTerm?.id || 1;
    const stepRows = await db.queryAll<{ id: number; sort_order: number }>(
