...
const APPLICANT_TYPES: string[] = [
  ...Array(50).fill('First-Time Freshman'),
  ...Array(35).fill('Transfer'),
  ...Array(15).fill('Readmit'),
];

const INTEGRATION_ACTORS: string[] = ['PeopleSoft Dev', 'CRM Import', 'Admissions Bot', 'system'];
...
function randomDateBetween(daysAgoStart: number, daysAgoEnd: number): Date {
  const now = Date.now();
...
// ── Batch insert helper ────────────────────────────────────────────────
async function batchInsert(db: Db, sql: string, rows: unknown[][], batchSize: number = 100): Promise<number> {
  let inserted = 0;
...
async function main(): Promise<void> {
  ...
  try {
    ...
    // ── Safety check ─────────────────────────────────────────────────
    if (!force) {
      const row = await db.queryOne<{ count: number }>('SELECT COUNT(*)::int AS count FROM students');
      if (row && row.count > 100) {
        const ok = await confirm(`⚠ Database already has ${row.count} students. Continue? (y/N) `);
        if (!ok) {
          console.log('Aborted.');
          await db.end();
          process.exit(0);
    ...
    const activeTerm = await db.queryOne<{ id: number; name: string }>('SELECT id, name FROM terms WHERE is_active = 1 LIMIT 1');
    if (!activeTerm) {
    ...
    const requiredSteps = await db.queryAll<{ id: number; title: string; sort_order: number }>(
      'SELECT id, title, sort_order FROM steps WHERE is_optional = 0 AND is_active = 1 AND term_id = $1 ORDER BY sort_order',
      [activeTerm.id]
    );
    const optionalSteps = await db.queryAll<{ id: number; title: string }>(
    ...
    const studentMeta: StudentMeta[] = []; // keep name + createdAt for later
