...
async function tick(db) {
  ...
  const progressRows = await db.queryAll(
  ...
  const notCompletedRequired = requiredSteps.filter(s => !progressMap.has(s.step_id ?? s.id) || progressMap.get(s.step_id ?? s.id) === 'not_completed');
...
export function startSimulator(db) {
  ...
  const scheduleNext = () => {
    ...
    setTimeout(async () => {
      try { await tick(db); } catch (err) { console.error('[simulator] Error:', err.message); }
      scheduleNext();
    }, delay);
  };
  scheduleNext();
